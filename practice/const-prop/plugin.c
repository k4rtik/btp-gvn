	...
	datastore * const_var;
                ...
                for (gsi = gsi_start_bb (bb); !gsi_end_p (gsi); gsi_next (&gsi))
		{
			if ( is_gimple_assign(gsi_stmt(gsi)) ) {
				...
				if (!DECL_ARTIFICIAL(lhs)) {/* skip autogenerated variables */
					if (TREE_CODE(rhs) == INTEGER_CST) {
						int val = ((TREE_INT_CST_HIGH (rhs) << HOST_BITS_PER_WIDE_INT) + TREE_INT_CST_LOW (rhs));
						insert_entry(lhs, val, rhs);
					}
					else if (TREE_CODE(rhs) == VAR_DECL) {
						if ((const_var = get_cst_var(rhs))) {
